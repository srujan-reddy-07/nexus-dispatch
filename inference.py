...
def get_llm_action(client, model, obs) -> Action:
    prompt = f"Expert Dispatcher. Rules: Match Medical:Ambulance, Fire:FireTruck, Police:PoliceUnit, Rescue:RescueTeam. Optimize distance. Obs: {obs.model_dump()}. Respond ONLY JSON: {{\"type\": \"DISPATCH\", \"unit_id\": \"UNIT-XX\", \"call_id\": \"CALL-XX\"}}"
    try:
...
def run_task(sim, client, model, name, diff, max_s):
    ...
    for _ in range(max_s):
        ...
        rewards.append(r)
        print(f"[STEP] step={len(rewards)} action={format_action(move)} reward={r:.2f} done={'true' if d else 'false'} error=null", flush=True)
        if d: break
